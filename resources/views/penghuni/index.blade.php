<x-app-layout>
    ...
    <div class="mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-8 max-w-7xl">

        {{-- Card Putih Utama --}}
        <div x-data="penghuniPage()" class="space-y-6 bg-white shadow-lg p-6 sm:p-8 rounded-[30px]">
                        ...
                        <table class="w-full text-sm">
                            ...
                            <tbody>
                                @foreach ($penghunis as $index => $penghuni)
                                    <tr class="hover:bg-grayCustom-50 border-grayCustom-100 border-b last:border-b-0 transition">
            ...
            {{-- ===================== MODAL: DETAIL ===================== --}}
            <div x-show="showDetail" x-cloak class="z-40 fixed inset-0 flex justify-center items-center bg-black/40 p-4" @click.self="showDetail = false">
                <div x-show="showDetail" x-transition class="bg-white shadow-xl p-6 rounded-2xl w-full max-w-sm">
                    ...
                    <template x-if="selected">
                        <dl class="space-y-3 text-sm">
                            ...
                            <div class="flex justify-between items-center">
                                ...
                                <dd>
                                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full font-semibold text-xs" :class="statusBadgeClass(selected.status)">
                    ...
                    <div class="flex justify-end mt-6">
                        <button type="button" @click="showDetail = false" class="hover:bg-grayCustom-100 px-4 py-2 rounded-xl font-semibold text-grayCustom-500 text-sm transition">Tutup</button>
                    </div>
                </div>
            </div>
            ...
            {{-- ===================== MODAL: TAMBAH ===================== --}}
            @can('penghuni.create')
                <div x-show="showAdd" x-cloak class="z-40 fixed inset-0 flex justify-center items-center bg-black/40 p-4" @click.self="showAdd = false">
                    <div x-show="showAdd" x-transition class="bg-white shadow-xl p-6 rounded-2xl w-full max-w-md">
                        ...
                        <form method="POST" action="{{ route($routePrefix . '.penghuni.store') }}" class="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="nama" value="{{ old('nama') }}" required placeholder="Masukkan nama penghuni" class="shadow-sm border-grayCustom-200 focus:border-primary rounded-xl focus:ring-primary w-full text-sm">
                            </div>
                            <div class="gap-4 grid grid-cols-2">
                                <div>
                                    <label class="block mb-1 font-medium text-grayCustom-500 text-xs">Nomor Kamar</label>
                                    <input type="text" name="nomor_kamar" value="{{ old('nomor_kamar') }}" required placeholder="Contoh : A01" class="shadow-sm border-grayCustom-200 focus:border-primary rounded-xl focus:ring-primary w-full text-sm">
                                </div>
                                <div>
                                    <label class="block mb-1 font-medium text-grayCustom-500 text-xs">No HP</label>
                                    <input type="text" name="no_hp" value="{{ old('no_hp') }}" required placeholder="08xxxxxxxxxx" class="shadow-sm border-grayCustom-200 focus:border-primary rounded-xl focus:ring-primary w-full text-sm">
                                </div>
                            </div>
                            <div class="gap-4 grid grid-cols-2">
                                <div>
                                    <label class="block mb-1 font-medium text-grayCustom-500 text-xs">Check In</label>
                                    <input type="date" name="check_in" value="{{ old('check_in') }}" required class="shadow-sm border-grayCustom-200 focus:border-primary rounded-xl focus:ring-primary w-full text-sm">
                                </div>
                                <div>
                                    <label class="block mb-1 font-medium text-grayCustom-500 text-xs">Status</label>
                                    <select name="status" required class="shadow-sm border-grayCustom-200 focus:border-primary rounded-xl focus:ring-primary w-full text-sm">
            ...
            {{-- ===================== MODAL: EDIT & HAPUS ===================== --}}
            @can('penghuni.edit')
                <div x-show="showEdit" x-cloak class="z-40 fixed inset-0 flex justify-center items-center bg-black/40 p-4" @click.self="showEdit = false">
                    <div x-show="showEdit" x-transition class="bg-white shadow-xl p-6 rounded-2xl w-full max-w-md">
                        ...
                        <template x-if="selected">
                            <form method="POST" id="formEditPenghuni" class="space-y-4">
                                ...
                                <div>
                                    <label class="block mb-1 font-medium text-grayCustom-500 text-xs">Nama</label>
                                    <input type="text" name="nama" x-model="selected.nama" required class="shadow-sm border-grayCustom-200 focus:border-primary rounded-xl focus:ring-primary w-full text-sm">
                                </div>
                                <div class="gap-4 grid grid-cols-2">
                                    <div>
                                        <label class="block mb-1 font-medium text-grayCustom-500 text-xs">Nomor Kamar</label>
                                        <input type="text" name="nomor_kamar" x-model="selected.nomor_kamar" required class="shadow-sm border-grayCustom-200 focus:border-primary rounded-xl focus:ring-primary w-full text-sm">
                                    </div>
                                    <div>
                                        <label class="block mb-1 font-medium text-grayCustom-500 text-xs">No HP</label>
                                        <input type="text" name="no_hp" x-model="selected.no_hp" required class="shadow-sm border-grayCustom-200 focus:border-primary rounded-xl focus:ring-primary w-full text-sm">
                                    </div>
                                </div>
                                <div class="gap-4 grid grid-cols-2">
                                    <div>
                                        <label class="block mb-1 font-medium text-grayCustom-500 text-xs">Check In</label>
                                        <input type="date" name="check_in" x-model="selected.check_in" required class="shadow-sm border-grayCustom-200 focus:border-primary rounded-xl focus:ring-primary w-full text-sm">
                                    </div>
                                    <div>
                                        <label class="block mb-1 font-medium text-grayCustom-500 text-xs">Status</label>
                                        <select name="status" x-model="selected.status" required class="shadow-sm border-grayCustom-200 focus:border-primary rounded-xl focus:ring-primary w-full text-sm">
                                            <option value="Active">Active</option>
                                            <option value="Inactive">Inactive</option>
                                        </select>
                                    </div>
                                ...
                                <div class="flex justify-between items-center pt-2">
                                    @can('penghuni.delete')
                                        <button type="button" @click="openDelete(selected)" class="hover:bg-red-50 px-3 py-2 rounded-xl font-semibold text-red-600 text-xs transition">
                                            Hapus Penghuni
                                        </button>
                                    @else
                                        <div></div>
            ...
            @can('penghuni.delete')
                <div x-show="confirmDelete" x-cloak class="z-50 fixed inset-0 flex justify-center items-center bg-black/50 p-4" @click.self="confirmDelete = false">
                    <div x-show="confirmDelete" x-transition class="bg-white shadow-xl p-6 rounded-2xl w-full max-w-xs text-center">
                        ...
                        <p class="text-grayCustom-700 text-sm">
                            Yakin ingin menghapus data penghuni <span class="font-semibold text-grayCustom-900" x-text="selected ? selected.nama : '-'"></span>?
                        </p>
        ...
        function penghuniPage() {
            return {
                ...
                statusBadgeClass(status) {
                    return status === 'Active' ? 'bg-success-100 text-success-700' : 'bg-grayCustom-100 text-grayCustom-600';
                },

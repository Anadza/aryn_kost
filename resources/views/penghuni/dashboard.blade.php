        ...
        <div class="bg-white shadow-sm p-6 rounded-xl">
            ...
            <div class="gap-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <a href="{{ route('penghuni.pengaduan.index') }}"
                    ...
                    <div class="flex justify-center items-center bg-orange-100 rounded-lg w-12 h-12 text-orange-600 group-hover:scale-105 transition">
                ...
                <a href="{{ route('penghuni.pengaduan.create') }}"
                    class="group flex items-center gap-4 bg-gray-50/50 hover:bg-gray-50 p-4 border border-gray-100 rounded-xl transition">
                    <div class="flex justify-center items-center bg-red-100 rounded-lg w-12 h-12 text-red-600 group-hover:scale-105 transition">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                            stroke="currentColor" class="w-6 h-6">
                    ...
                    <div class="flex-1 space-y-2">
                        @if ($penghuniData)
                            ...
                            <div class="gap-x-2 gap-y-1 grid grid-cols-2 text-sm">
                                ...
                                <span class="font-medium text-gray-800">{{ \Illuminate\Support\Carbon::parse($penghuniData->check_in)->translatedFormat('d F Y') }}</span>
                                <span class="text-gray-500">Tipe Kamar</span>
                                <span class="font-medium text-gray-800">{{ $kamarSaya->tipe ?? '-' }}</span>
                            </div>
                        @else
                            <p class="text-gray-500 text-sm">Data kamar kamu belum terhubung. Hubungi admin kos untuk memastikan data penghuni sudah sesuai.</p>
                        @endif
                    </div>
                </div>
            </div>

            <!-- Tagihan Bulan Ini (Dinamis & Terkoneksi) -->
            <div class="bg-white shadow-sm p-6 rounded-xl">
                <div class="flex justify-between items-start">
                    <h2 class="font-semibold text-primary text-lg">Tagihan Bulan Ini</h2>
                    @if ($tagihanAktif)
                        <span class="text-gray-500 text-sm font-medium">{{ $tagihanAktif->bulan_tagihan }}</span>
                    @endif
                </div>
                ...
                @if ($tagihanAktif)
                    ...
                    <div class="flex justify-between mt-4 text-sm">
                        ...
                        <span class="font-medium text-gray-800">
                            {{ \Illuminate\Support\Carbon::parse($tagihanAktif->tanggal_jatuh_tempo)->translatedFormat('d F Y') }}
                        </span>
                    </div>

                    {{-- Tombol Kondisional berdasarkan Status Pembayaran --}}
                    @if ($tagihanAktif->status_pembayaran === 'Belum Dibayar')
                        <a href="{{ route('penghuni.pembayaran.upload') }}"
                            class="block bg-primary hover:bg-primary/90 mt-5 px-4 py-2.5 rounded-lg w-full font-semibold text-white text-sm text-center transition shadow-sm">
                            Bayar Sekarang
                        </a>
                    @elseif($tagihanAktif->status_pembayaran === 'Menunggu Konfirmasi')
                        <div class="mt-4 p-3 bg-yellow-50 rounded-lg border border-yellow-100 text-xs text-yellow-800">
                            <p class="font-medium">Bukti transfer sudah dikirim!</p>
                            <p class="text-yellow-600 mt-0.5">Mohon tunggu admin mengecek pembayaran Anda.</p>
                        </div>
                    @else
                        <div class="mt-4 p-3 bg-green-50 rounded-lg border border-green-100 text-xs text-green-800 text-center font-medium">
                            ✓ Tagihan bulan ini telah diselesaikan
                        </div>
                    @endif
                @else
                    {{-- Tampilan jika benar-benar bersih / tidak ada data tunggakan --}}
                    <div class="flex flex-col items-center justify-center text-center py-8">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-12 h-12 text-green-500 mb-2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                        </svg>
                        <p class="text-gray-500 text-sm font-medium">Semua tagihan Anda lunas.</p>
                    </div>
                @endif
            </div>
        </div>

        <!-- Riwayat Pembayaran Terakhir (Dinamis) -->
        <div class="bg-white shadow-sm p-6 rounded-xl">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($riwayatPembayaran as $tagihan)
                            ...
                            <tr class="hover:bg-gray-50/50 transition">
                                <td class="py-4 pr-4">
                                    <p class="font-semibold text-gray-800">{{ $tagihan->bulan_tagihan }}</p>
                                    <p class="text-gray-500 text-xs">Tagihan Bulanan</p>
                                </td>
                                <td class="py-4 pr-4">
                                    ...
                                    <p class="text-gray-500 text-xs">
                                        @if($tagihan->tanggal_bayar)
                                            Dibayar: {{ \Illuminate\Support\Carbon::parse($tagihan->tanggal_bayar)->translatedFormat('d M Y, H:i') }}
                                        @else
                                            Batas: {{ \Illuminate\Support\Carbon::parse($tagihan->tanggal_jatuh_tempo)->translatedFormat('d M Y') }}
                                        @endif
                                    </p>
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="py-8 text-gray-400 text-center">Belum ada riwayat pembayaran tercatat.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            @if ($riwayatPembayaran->hasPages())
                <div class="flex justify-end items-center gap-2 mt-4 pt-4 border-gray-100 border-t text-xs">

                    {{-- Tombol Sebelumnya --}}
                    @if (!$riwayatPembayaran->onFirstPage())
                        <a href="{{ $riwayatPembayaran->appends(request()->query())->previousPageUrl() }}"
                            class="hover:bg-gray-100 px-2 py-1 border border-gray-200 rounded text-gray-600 transition">
                            ←
                        </a>
                    @endif
